...
class HudClientTests(unittest.TestCase):
    ...
    def test_t24_extracts_only_second_column_from_each_row(self):
        response = (
            "t24_Result:1 11 101 102 103 104 105,"
            "2 21 201 202 203 204 205,%"
        )
        self.assertEqual(parse_t24_rows(response), [[11], [21]])

    def test_exports_t24_second_column_to_excel(self):
        from hud_client import TestResult, export_t24_excel
        ...
        results = [
            TestResult(
                "11",
                "t24",
                "t24_Result:1 11 101 102 103 104 105,2 12 201 202 203 204 205,%",
            ),
            TestResult(
                "22",
                "t24",
                "t24_Result:1 21 101 102 103 104 105,2 22 201 202 203 204 205,%",
            ),
        ]
        with TemporaryDirectory() as directory:
            output = export_t24_excel(results, str(Path(directory) / "result.xlsx"))
            sheet = load_workbook(output, data_only=True)["t24测试结果"]
            self.assertEqual(sheet["A1"].value, "11（配置文件名称）")
            self.assertEqual([sheet["A2"].value, sheet["A3"].value], [11, 12])
            self.assertEqual(sheet["C1"].value, "22（配置文件名称）")
            self.assertEqual([sheet["C2"].value, sheet["C3"].value], [21, 22])
